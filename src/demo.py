...
async def async_handle_video(camInfo):
    ...
    global emotion_list  # Emotion labels ["angry", "sad", "happy", "pleased", "neutral"]
    global emot_region
...
async def handle_video_analysis():    
    def hand_detection():
        ...
        if do_hand_gesture == 1:
            ...
            latent_feature = encoder(cropped_face_th_norm.unsqueeze(0).type(torch.FloatTensor).to('cpu'))
...
class Config():
    def __init__(self):
        ...
        self.set_ini_config('/home/orinz/Documents/ELIM/FER_ELIM-main/config.ini') #Change your path
...
class FER_INT_ALG():
    def __init__(self):
        ...
        protoPath = "/home/orinz/Documents/ELIM/FER_ELIM-main/face_detector/deploy.prototxt"
        modelPath = "/home/orinz/Documents/ELIM/FER_ELIM-main/face_detector/res10_300x300_ssd_iter_140000.caffemodel"
        net = cv2.dnn.readNetFromCaffe(protoPath, modelPath)
        
        global encoder, regressor, task_header
        #Change your path
        encoder, regressor, task_header = nn_output()
        encoder.load_state_dict(torch.load('/home/orinz/Documents/ELIM/FER_ELIM-main/weights/enc2.t7', map_location=torch.device('cpu'), weights_only=True), strict=False)
        regressor.load_state_dict(torch.load('/home/orinz/Documents/ELIM/FER_ELIM-main/weights/reg2.t7', map_location=torch.device('cpu'), weights_only=True), strict=False)
        task_header.load_state_dict(torch.load('/home/orinz/Documents/ELIM/FER_ELIM-main/weights/header2.t7', map_location=torch.device('cpu'), weights_only=True), strict=False)
